package Presentation.UserView.AdminView.Revenue.EmployeeRevenue;
...
public class EmployeeRevenue extends JPanel {
    JLabel title = new JLabel("Employee Revenue");
    MyButton sortBtn = new MyButton("Sort");
    EmployeeRevenueBU bu = new EmployeeRevenueBU();
    ...
    public EmployeeRevenue() {
        ...
        List<UserPOJO> employees = employeeBU.getAll();
        String book[] = new String[employees.size()];

        int k = 0;
        for (int i = 0; i < employees.size(); i++) {
            if(employees.get(i).getRole_id()==2){
            book[k] = employees.get(i).getUsername();
            k++;
            }
        }
        ...
        bookName.setBounds(135, 82, 230, 26);
