package Presentation.UserView.AdminView.Revenue.CustomerRevenue;

import Business.BookBU;
import Business.CustomerBU;
import Business.CustomerRevenueBU;
...
public class CustomerRevenue extends JPanel {
    JLabel title = new JLabel("Customer Revenue");
    MyButton sortBtn = new MyButton("Sort");
    CustomerRevenueBU bu = new CustomerRevenueBU();

    JLabel name = new JLabel("Tel");
    ...
    public CustomerRevenue() {
        ...
        List<CustomerPOJO> customers = customerBU.getAll();
        String book[] = new String[customers.size()];

        for (int i = 0; i < customers.size(); i++) {
            book[i] = customers.get(i).getTel();
        }
